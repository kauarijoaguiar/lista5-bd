...
JOIN PIZZASABOR ON PIZZA.CODIGO=PIZZASABOR.PIZZA
JOIN SABOR ON PIZZASABOR.SABOR = SABOR.CODIGO
WHERE  DATE(COMANDA.DATA) BETWEEN DATETIME('2021-03-20 06:38:00') 
AND DATETIME('2021-06-21 00:32:00')
GROUP BY SABOR.CODIGO
HAVING QUANTIDADE <= (
...
SELECT distinct SABOR1.NOME as sabor1, sabor2.nome as sabor2, COUNT(*) as pedidos
    from comanda
        join pizza on pizza.comanda = comanda.numero
        join pizzasabor pizzasabor1 on pizzasabor1.pizza = pizza.codigo
        join pizzasabor pizzasabor2 on pizzasabor2.pizza = pizza.codigo
        join sabor sabor1 on pizzasabor1.sabor = sabor1.codigo
        join sabor sabor2 on pizzasabor2.sabor = sabor2.codigo
    where date(comanda.data) BETWEEN DATE('now', '-3 months', 'localtime') 
	AND DATE('now', 'localtime') and sabor1.codigo <> sabor2.codigo
group by sabor1.codigo, sabor2.codigo
order by pedidos desc
limit 1;


--O)
SELECT NOMES AS SABORES, QUANTIDADEPEDIDOS FROM 
    (SELECT SABORES, NOMES, COUNT(*) AS QUANTIDADEPEDIDOS FROM
        (SELECT PIZZASABOR.PIZZA, GROUP_CONCAT(PIZZASABOR.SABOR) AS SABORES, 
        GROUP_CONCAT(SABOR.NOME) AS NOMES FROM SABOR
            JOIN PIZZASABOR ON PIZZASABOR.SABOR = SAbOR.CODIGO
            JOIN PIZZA ON PIZZASABOR.PIZZA = PIZZA.CODIGO
            JOIN COMANDA ON PIZZA.COMANDA=COMANDA.NUMERO
        WHERE COMANDA.DATA BETWEEN DATE('NOW', '-3 MONTHS', 'LOCALTIME') AND DATE('NOW')
        AND PIZZASABOR.PIZZA IN 
            (SELECT PIZZASABOR.PIZZA FROM SABOR
                JOIN PIZZASABOR ON PIZZASABOR.SABOR = SABOR.CODIGO
                JOIN PIZZA ON PIZZASABOR.PIZZA= PIZZA.CODIGO
                JOIN COMANDA ON PIZZA.COMANDA= COMANDA.NUMERO
            WHERE COMANDA.DATA BETWEEN DATE ('NOW', '-3 MONTHS', 'LOCALTIME') AND DATE('NOW')
            GROUP BY PIZZASABOR.PIZZA
            HAVING COUNT(PIZZASABOR.PIZZA)= 5)
        GROUP BY PIZZASABOR.PIZZA)
    GROUP BY SABORES
