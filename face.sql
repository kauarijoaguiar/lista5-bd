...
WHERE DATE(POST.DATAPOST, 'localtime') 
BETWEEN DATE ('now', '-30 days', 'localtime' ) 
 AND DATE ('now', 'localtime') AND POST.PAIS = 'EUA'
GROUP BY ASSUNTO.ASSUNTO, POST.PAIS 
UNION SELECT POST.PAIS AS PAIS, ASSUNTO.ASSUNTO AS ASSUNTO, COUNT(ASSUNTO) AS QUANTIDADE  FROM ASSUNTO
    JOIN ASSUNTOPOST ON ASSUNTOPOST.CODIGOASSUNTO = ASSUNTO.CODIGO
    JOIN POST ON ASSUNTOPOST.CODIGOPOST = POST.CODIGO
WHERE DATE(POST.DATAPOST, 'localtime') 
BETWEEN DATE ('now', '-30 days', 'localtime' ) 
 AND DATE ('now', 'localtime') AND POST.PAIS = 'Cuba'
GROUP BY ASSUNTO.ASSUNTO, POST.PAIS ;
...
SELECT ASSUNTO.ASSUNTO AS ASSUNTO, COUNT(ASSUNTO) AS QUANTIDADE FROM ASSUNTO
    JOIN ASSUNTOPOST ON ASSUNTOPOST.CODIGOASSUNTO = ASSUNTO.CODIGO
    JOIN POST ON ASSUNTOPOST.CODIGOPOST = POST.CODIGO
WHERE DATETIME(DATAPOST) BETWEEN DATETIME('2021-07-01 00:00:00') 
AND DATETIME('2021-07-31 23:59:59') AND POST.PAIS = 'Brasil'
GROUP BY ASSUNTO.CODIGO IN (
SELECT COUNT(ASSUNTO) AS QUANTIDADE FROM ASSUNTO
    JOIN ASSUNTOPOST ON ASSUNTOPOST.CODIGOASSUNTO = ASSUNTO.CODIGO
    JOIN POST ON ASSUNTOPOST.CODIGOPOST = POST.CODIGO
WHERE  DATETIME(DATAPOST) BETWEEN DATETIME('2021-06-01 00:00:00') 
AND DATETIME('2021-06-30 23:59:59') AND POST.PAIS = 'Brasil' 
	GROUP BY ASSUNTO.ASSUNTO
)	ORDER BY QUANTIDADE DESC
LIMIT 5;
