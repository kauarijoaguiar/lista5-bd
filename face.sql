...
WITH USUARIOSDOSGRUPOS AS (
    SELECT DISTINCT USUARIO.EMAIL FROM GRUPO
    JOIN GRUPOUSUARIO ON GRUPOUSUARIO.CODIGOGRUPO = GRUPO.CODIGO
    JOIN USUARIO ON GRUPOUSUARIO.EMAIL_USUARIO = USUARIO.EMAIL
    WHERE GRUPO.NOMEGRUPO='SQLite' OR GRUPO.NOMEGRUPO='Banco de Dados-IFRS-2021'
)
SELECT DISTINCT AMIGOSUSUARIO1.USUARIOGRUPO, AMIGOSUSUARIO2.USUARIOGRUPO, COUNT(*)/2 AS TOTALAMIGOSCOMUM FROM
(
    SELECT EMAIL_USUARIO1 EMAIL_USUARIO, EMAIL_USUARIO2 AS USUARIOGRUPO FROM AMIZADE WHERE EMAIL_USUARIO2 IN USUARIOSDOSGRUPOS
    UNION 
    SELECT EMAIL_USUARIO2 EMAIL_USUARIO, EMAIL_USUARIO1 AS USUARIOGRUPO FROM AMIZADE WHERE EMAIL_USUARIO1 IN USUARIOSDOSGRUPOS
) AS AMIGOSUSUARIO1
JOIN  
(
    SELECT EMAIL_USUARIO2 EMAIL_USUARIO, EMAIL_USUARIO1 AS USUARIOGRUPO FROM AMIZADE WHERE EMAIL_USUARIO1 IN USUARIOSDOSGRUPOS
...
ON  AMIGOSUSUARIO1.EMAIL_USUARIO = AMIGOSUSUARIO2.EMAIL_USUARIO
WHERE AMIGOSUSUARIO2.USUARIOGRUPO <>  AMIGOSUSUARIO1.USUARIOGRUPO
GROUP BY AMIGOSUSUARIO1.EMAIL_USUARIO
ORDER BY AMIGOSUSUARIO2.USUARIOGRUPO <>  AMIGOSUSUARIO1.USUARIOGRUPO;

--L)
SELECT u1.NOME AS USUARIOA, u2.NOME AS USUARIOB
FROM USUARIO u1,USUARIO u2
JOIN (SELECT POST.EMAIL_USUARIO, ASSUNTO.CODIGO FROM ASSUNTO
    JOIN ASSUNTOPOST ON ASSUNTOPOST.CODIGOASSUNTO = ASSUNTO.CODIGO
    JOIN POST ON ASSUNTOPOST.CODIGOPOST = POST.CODIGO
WHERE DATE(POST.DATAPOST, 'localtime') BETWEEN DATE ('now', '-3 months', 'localtime') 
AND DATE ('now', 'localtime')
