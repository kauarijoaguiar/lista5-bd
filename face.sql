...
--D)
SELECT POST.PAIS AS PAIS, ASSUNTO.ASSUNTO, COUNT(*) FROM ASSUNTO
    JOIN ASSUNTOPOST ON ASSUNTOPOST.CODIGOASSUNTO = ASSUNTO.CODIGO
    JOIN POST ON ASSUNTOPOST.CODIGOPOST = POST.CODIGO
WHERE DATE(POST.DATAPOST, 'localtime') 
BETWEEN DATE ('now', '-120 days', 'localtime' ) 
 AND DATE ('now', 'localtime')
GROUP BY POST.PAIS, ASSUNTO.ASSUNTO
...
(SELECT DISTINCT TMP.EMAIL, POST.CODIGO, TMP.QTDCOMENTARIOS FROM USUARIO
JOIN GRUPOUSUARIO ON GRUPOUSUARIO.EMAIL_USUARIO = USUARIO.EMAIL
JOIN GRUPO ON GRUPOUSUARIO.CODIGOGRUPO = GRUPO.CODIGO
JOIN POST ON POST.EMAIL_USUARIO = USUARIO.EMAIL
JOIN ASSUNTOPOST ON ASSUNTOPOST.CODIGOPOST = POST.CODIGO
JOIN ASSUNTO ON ASSUNTOPOST.CODIGOASSUNTO = ASSUNTO.CODIGO
JOIN (SELECT DISTINCT USUARIO.EMAIL, POST.CODIGO, COUNT(*) AS QTDCOMENTARIOS FROM USUARIO
    JOIN GRUPOUSUARIO ON GRUPOUSUARIO.EMAIL_USUARIO = USUARIO.EMAIL
    JOIN GRUPO ON GRUPOUSUARIO.CODIGOGRUPO = GRUPO.CODIGO
    JOIN POST ON POST.EMAIL_USUARIO = USUARIO.EMAIL
    JOIN ASSUNTOPOST ON ASSUNTOPOST.CODIGOPOST = POST.CODIGO
    JOIN ASSUNTO ON ASSUNTOPOST.CODIGOASSUNTO = ASSUNTO.CODIGO
    JOIN POST COMENTARIO ON POST.CODIGO= COMENTARIO.CODPOSTREFERENCIA
    WHERE GRUPO.NOMEGRUPO='SQLite' AND POST.CODPOSTREFERENCIA IS NULL
    GROUP BY POST.CODIGO
    ) TMP on TMP.EMAIL = USUARIO.EMAIL AND POST.CODIGO=TMP.CODIGO
WHERE GRUPO.NOMEGRUPO='SQLite' AND POST.CODPOSTREFERENCIA IS NULL AND ASSUNTO.ASSUNTO='SELECT'
ORDER BY TMP.QTDCOMENTARIOS DESC
LIMIT 3) CONSULTA
JOIN USUARIO ON CONSULTA.EMAIL = USUARIO.EMAIL;


--J)
WITH USUARIOSDOSGRUPOS AS (
    SELECT USUARIO.EMAIL FROM GRUPO
    JOIN GRUPOUSUARIO ON GRUPOUSUARIO.CODIGOGRUPO = GRUPO.CODIGO
    JOIN USUARIO ON GRUPOUSUARIO.EMAIL_USUARIO = USUARIO.EMAIL
    WHERE GRUPO.NOMEGRUPO='SQLite' OR GRUPO.NOMEGRUPO='Banco de Dados-IFRS-2021'
)
SELECT CASE WHEN USUARIO1.EMAIL IN USUARIOSDOSGRUPOS THEN USUARIO1.NOME
ELSE USUARIO2.NOME 
END AS NOMEUSUARIO, COUNT(*) AS NUMEROAMIGOS
    FROM AMIZADE
...
WHERE USUARIO1.EMAIL IN USUARIOSDOSGRUPOS OR USUARIO2.EMAIL IN USUARIOSDOSGRUPOS
